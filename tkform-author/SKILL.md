...
**Structural correctness** comes from the bundled JSON Schema (`tkform.schema.json`, registered for autocomplete in `package.json`). **Behavioral correctness** comes from the engine validator (`scripts/validate_project.py`). Read the references on demand for details; this file is the routing hub.
...
The script uses the SAME engine the VS Code extension uses, so passing it = the user will see a clean Problems panel when they open the file.

## Useful patterns from the examples

The repo ships three reference projects in `examples/` — read them when the user's request resembles one:

...
The engine validator (`python/tkform_engine/project_validation.py`) is the single source of truth for what is a valid tkform project. **Always run it after authoring** — the JSON Schema (`tkform.schema.json`) only checks structural shape; cross-references, code compilation, and the rules below need the engine.
...
The bundled `tkform.schema.json` (registered via `package.json` jsonValidation) gives editor autocomplete and basic shape checks. It does **not** replace engine validation:
